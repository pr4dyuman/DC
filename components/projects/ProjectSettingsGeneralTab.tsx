            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    {selectedClientIds.length > 0 && !isEditingSelection && (
                        <Button variant="ghost" size="sm" onClick={onStartEditingSelection} className="h-8 gap-2 text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50">
                        ...
                        {clients.filter((c) => selectedClientIds.includes(c.id)).map((c) => (
                            <div key={c.id} className="bg-primary/10 border border-primary/20 rounded-xl p-4 flex items-start gap-3">
                                ...
                                <div className="space-y-0.5">
                                    ...
                                    <div className="flex items-center gap-2 text-sm text-muted-foreground"><Users className="h-3 w-3" />{c.name}</div>
                                    <div className="flex items-center gap-2 text-sm text-muted-foreground"><Mail className="h-3 w-3" />{c.email}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="space-y-3">
                            <div className="grid gap-2">
                                ...
                                <div className="max-h-48 overflow-y-auto rounded-md border border-input bg-background p-2 space-y-1">
                                    {clients.length === 0 && <p className="text-sm text-muted-foreground px-1">No clients yet</p>}
                                    {clients.map((client) => {
                                        const isChecked = selectedClientIds.includes(client.id);
                                        return (
                                            <label key={client.id} className="flex items-center gap-2 cursor-pointer rounded px-2 py-1.5 hover:bg-muted text-sm">
                                                <input
                                                    type="checkbox"
                                                    checked={isChecked}
