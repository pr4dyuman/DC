...
/**
 * Initialize and return a configured Brevo API client
 */
...
function getConfiguredSender() {
  const email = process.env.BREVO_SENDER_EMAIL || process.env.ADMIN_EMAIL;
  const name = process.env.BREVO_SENDER_NAME || "Digital Corvids";

  if (!email) {
    throw new Error("BREVO_SENDER_EMAIL or ADMIN_EMAIL must be set in environment variables");
  }
...
export async function sendAdminNotificationEmail(formData) {
  ...
  const sendSmtpEmail = {
    ...
    sender: {
      name: process.env.BREVO_SENDER_NAME || "Digital Corvids Website",
      email: sender.email,
    },
    htmlContent: `
    <!DOCTYPE html>
