...
/**
 * Syncs a new budget value across all service payment configs for an existing project.
 * - Budget > 0: distributes equally as installmentAmount; marks paymentDetailsLater=false
 * - Budget = 0: marks all services as paymentDetailsLater=true (clears the deal value)
 * This is used by the "Edit Deal Value" field in Project Settings.
 */
export async function syncProjectBudgetImpl(
    ...
    const projectDoc = await ProjectModel.findOne({ id: projectId, agencyId })
        .select("id services serviceConfigs budget")
        .lean() as ProjectServiceOwnerSnapshot & { budget?: number } | null;
    if (!projectDoc) throw new Error("Project not found");

    const serviceConfigs = (projectDoc.serviceConfigs || []) as Array<{
        serviceId: string;
        name: string;
        paymentConfig?: PaymentConfig;
    }>;
    ...
    if (serviceConfigs.length === 0) {
        throw new Error("Cannot set deal value: project has no services. Add at least one service first.");
    }

    const perServiceAmount = newBudget > 0
        ? Math.round(newBudget / serviceConfigs.length)
        : 0;
    ...
    const updatedConfigs = serviceConfigs.map((cfg) => ({
        ...
        paymentConfig: {
            ...(cfg.paymentConfig || {}),
            type: cfg.paymentConfig?.type || "installment",
            paymentDetailsLater: newBudget === 0,
            installments: cfg.paymentConfig?.installments || 1,
            installmentAmount: perServiceAmount,
            monthlyAmount: cfg.paymentConfig?.type === "monthly"
                ? perServiceAmount
                : (cfg.paymentConfig?.monthlyAmount || 0),
        } as PaymentConfig,
    }));
