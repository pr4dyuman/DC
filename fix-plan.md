...
**Remaining (larger refactors):** BUG-038 (loading states), BUG-041 (empty states), BUG-053 (mobile polling), BUG-054 (mobile nav), BUG-055 (shared ChatContext), BUG-056 (heartbeat visibility), BUG-057 (touch DnD), BUG-069 (pagination), BUG-078 (super-admin settings), BUG-085 (fabricated logs), BUG-130 (confirm UX), BUG-233 (informational).
...
**Fixed (Round 3 - commit e0aead5):** BUG-064 (body size 10MB→2MB), BUG-091 (DocumentManager 2MB file size limit), BUG-101 (asset not found throws error), BUG-102 (TOCTOU username race with retry), BUG-108 (getCurrentAgency hoisted outside loop), BUG-109 (username compound unique index per agency), BUG-110 (client email compound unique index per agency), BUG-113 (Task.dueDate optional in schema), BUG-116 (blog slug duplicate handling), BUG-222 (NODE_ENV removed from client), BUG-281 (signup username uniqueness check)
...
**Remaining (larger refactors/infra):** BUG-021 (activity log name→ID), BUG-033 (AI args — already sanitized at server action level), BUG-034 (AI delete confirmation — design decision), BUG-047 (month calc approximation — already validated), BUG-063 (AI timeout — AbortController), BUG-066 (health check — new feature), BUG-068 (plan limits — new feature), BUG-072 (timezone — global refactor), BUG-073 (concurrent task status — atomic ops), BUG-074 (separate DB connection — architectural), BUG-075 (no Sentry — infrastructure), BUG-087 (mobile search — UI design), BUG-097 (fake upload — needs full upload system), BUG-134 (newsletter — no backend), BUG-264 (password in forms — unavoidable), BUG-271 (chat timeout — complex), BUG-287 (ESLint plugins — config only)
**These remaining items are either infrastructure-level decisions, new feature development, or architectural refactors that should be scoped as separate work items.**
